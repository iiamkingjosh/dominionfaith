  ...
  return (
    <div className="relative">
      <select
        ...
        className="w-full appearance-none rounded-xl border bg-transparent py-2.5 pl-4 pr-9 text-[13px] text-white/80 transition-colors focus:outline-none"
        style={{ borderColor: value ? 'rgba(42,47,170,0.5)' : 'rgba(255,255,255,0.12)', background: value ? 'rgba(42,47,170,0.08)' : 'rgba(255,255,255,0.04)' }}
        aria-label={label}
      >
...
export default function SermonArchivePage() {
  ...
  const results = useMemo(() => {
    ...
    else if (sort === 'most-viewed') list = [...list].sort((a, b) => (b.viewCount ?? 0) - (a.viewCount ?? 0))
    return list
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden py-28 md:py-36">
        <div className="pointer-events-none absolute -left-40 -top-40 h-[500px] w-[500px] rounded-full opacity-20"
        ...
        <div className="mx-auto max-w-3xl px-6 text-center md:px-16">
          <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.18em] text-white/40">Sermon Archive</p>
          <h1 className="mb-4 font-black text-white" style={{ fontSize: 'clamp(32px, 5vw, 64px)' }}>
      ...
      <div className="mx-auto max-w-7xl px-6 pb-24 md:px-16">
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <button
            ...
            style={{
              background: showFilters || activeFilterCount > 0 ? 'rgba(42,47,170,0.15)' : 'rgba(255,255,255,0.05)',
          ...
          >
            ...
            Filters{activeFilterCount > 0 && <span className="rounded-full bg-[#2A2FAA] px-1.5 py-0.5 text-[10px] font-black text-white">{activeFilterCount}</span>}
          </button>
        ...
        <p className="mb-6 text-[13px] text-white/40">
          ...
          {debouncedQuery && <span> for "<span className="text-white/60">{debouncedQuery}</span>"</span>}
        </p>
        ...
        {totalPages > 1 && (
          <div className="mt-12 flex items-center justify-center gap-2">
            <button
              ...
              disabled={page === 1}
              className="rounded-xl px-4 py-2 text-[13px] font-semibold text-white/50 disabled:opacity-30 transition-colors hover:text-white"
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
              <button
                ...
                style={{
                  background: n === page ? '#2A2FAA' : 'rgba(255,255,255,0.06)',
                  color: n === page ? 'white' : 'rgba(255,255,255,0.5)',
                }}
              >{n}</button>
            ))}
            <button
